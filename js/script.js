...
function fnUpdateStatusUserHousekeeping(
  ...
  fnSetColorByStatusUserHousekeeping(iconMsEleId, status.status);
}
//--------------------- call next process-------------------
...
function rpt_user_housekeeping_fn_refresh_listing(type, date) {
  ...
  if (tabProcess == "1001") {
    if (date === undefined || date === "") {
      let dateRage = fnGetFullMonthDate($("#rang_date_id_user_housekeeping").val()).split("~");
      fromDate = dateRage[0];
      toDate = dateRage[1];
...
// function fnGetUserBiCloseUserHousekeeping() {
//     let date = $("#bi_date_report_ele_id").val();
//     CallAPI.Go(
//         url_get_user_bi_close_user_housekeeping,
//         { date: date },
//         (data) => {
//             if (data.status == "1") {
...
function fnReqAuthBIHousekeeping() {}
...
// function rptDbHkpFnGetUserForInform(record_ids) {
//     try {
//         if (Array.isArray(record_ids.toString().split(","))) {
//             let arrRecordIds = record_ids.toString().split(",");
//             console.log(arrRecordIds);
//             let dataFilter = dbHkpListingData.filter((r) =>
//                 arrRecordIds.includes(r.id)
//             );
//             return dataFilter;
//         } else {
//             let dataFilter = dbHkpListingData.filter((r) => r.id === record_ids);
//             return dataFilter;
//         }
//     } catch (error) {
//         console.error(error);
...
function user_housekeeping_fn_get_bi_deletion_listing() {
    ...
    CallAPI.Go(url_get_bi_deletion_listing, { date: user_housekeeping_fn_get_correct_date_format(date_report) },
        (data) => {
          if(data.status === "1") {
...
function user_housekeeping_fn_get_bi_user_remove_listing() {
    let date_report = $("#rang_date_id_user_housekeeping").val();
    CallAPI.Go(v_user_housekeeping_get_user_inactive, { date: user_housekeeping_fn_get_correct_date_format(date_report) },
        (data) => {
            if (data.status === "1") {
...
/*
let statusSentEmail = false;
function RptBIHkpFnSentEmailInformUserInactive(userInform) {
    let reportDate = "";
    let type = "";
    if (tabProcess === "1001") {
        type = "1001";
        reportDate = $("#bi_date_report_ele_id").val();
    } else if (tabProcess === "2002") {
        type = "2002";
        reportDate = $("#db_date_report_id").val();
    }
    let date = fnGetFullMonthDate(reportDate);

    //CallAPI.Go(
    //    url_set_mail_inform_user_db_housekeeping,
    //    { reportDate: date.split("~")[0], type: type, userInform: userInform },
    //    RptBIHkpFnSentEmailInformUserInactiveCallBack,
    //    "Sending email.."
    //);

    setTimeout(function () {
        statusSentEmail = true;
    }, 1000);

    function insertProcessSentEmail() {
        rpt_user_housekeeping_fn_insert_process_step(
            "5",
            "Y",
            rpt_user_housekeeping_fn_get_date_time_now().toString(),
            "1001",
            "N/A"
        );
    }
    fnCallNextProcessUserHousekeeping(
        insertProcessSentEmail,
        () => statusSentEmail,
        500,
        5,
        "sent mail inform"
...
function fn_clear_all_by_tab_user_housekeeping(tab) {
 ...
 function user_housekeeping_fn_bi_tab_select(){
   ...
   var showElementIds = [
     ...
     "rpt_bi_get_user_close",
     "RptBIHkpInsertBIUserCloseBTN",
   ];
   var hideElementIds = [
     "RptBIHkpGenDBHkpBtn",
     "RptDBHkpAuthBTN",
   ...
   //update process step and status
 ...
 function user_housekeeping_fn_db_listing_tab_select(){
   ...
   $("#RptBIHkpRefreshBTN").attr(
     "onclick",
     "RptBIHkpFnRefreshListing('db_housekeeping','')"
   );
 
   // manual import => $("#RptBIHkpSentMailInformBTN").attr("onclick", "rptDBFnSentMailToUserHandle()");
 
   // set active listing tab
   $('.nav-tabs a[href="#href_tab_id_db_housekeeping"]').tab("show");
 ...
 function user_housekeeping_fn_bi_listing_tab_select(){
   ...
   $("#RptBIHkpRefreshBTN").attr(
     "onclick",
     "RptBIHkpFnRefreshListing('biInactive','getReportListing')"
   );
   // set active listing tab
   $('.nav-tabs a[href="#href_tab_id_bi_housekeeping"]').tab("show");
   dataTable.Recal();
 }
 function user_housekeeping_fn_bi_remove_listing_tab_select(){
   $("#RptBIHkpRefreshBTN").attr(
     "onclick",
     "user_housekeeping_fn_get_bi_user_remove_listing()"
   );
   $('.nav-tabs a[href="#RptBIHkpBIDeletionTab"]').tab("show");
   dataTable.Recal();
 }
 function user_housekeeping_fn_bi_deletion_listing_tab_select(){
   $("#RptBIHkpRefreshBTN").attr(
     "onclick",
 ...
 function fn_tab_change_user_housekeeping(btn_attr) {
   if (!statusCallFun) {
     ...
     if (btn_attr === "db_listing_tab") {
       user_housekeeping_fn_db_listing_tab_select();
     }
     if (btn_attr === "bi_listing_tab") {
       user_housekeeping_fn_bi_listing_tab_select();
     }
     // bi user close listing tab
     if (btn_attr === "pre_close_user_tab") {
       user_housekeeping_fn_bi_remove_listing_tab_select();
     }
     if (btn_attr === "bi_deletion_tab") {
       user_housekeeping_fn_bi_deletion_listing_tab_select();
     }
   }
   statusCallFun = true;
   setTimeout(() => {
 ...
 /*
 let dataUserClose = "";
 let staInsertStepLoadUserFromExcel = false;
 let staInsertProLoadUserFromExcel = false;
 let staInsertUserCloseFromExcel = false;
 //end get user from excel file
 // start insert user pre close
 function RptBIHkpInsertUserPreClose() {
   let userClose = "";
   //update status get data from excel
   isLoadDataFromExcel = false;
   ...
   // insert all user
   if (dataUserClose !== undefined) {
     dataUserClose.data.listStaff.forEach((element) => {
       userClose +=
         element.staffId.toString() +
         "~" +
         (element.typeClose === "" || element.typeClose === "USER RESIGNED"
           ? "USER RESIGNED"
           : element.typeClose) +
         "~" +
         date +
   ...
   } else if (oldUserPreClose !== "") {
     oldUserPreClose.data.listStaff.forEach((element) => {
       userClose +=
         element.staffId.toString() +
         "~" +
         (element.typeClose === "" || element.typeClose === "USER RESIGNED"
   ...
   CallAPI.Go(
     url_insert_user_bi_close,
     data,
     RptBIHkpInsertUserPreCloseCallback,
     "Processing"
   );
 }
 */
 ...
 /*
 var isInsertUserClose = false;
 var statusInsertStepUserClose = false;
 var statusInsertProcessUserClose = false;
 function RptBIHkpInsertUserPreCloseCallback(data) {
     if (data.status === "1") {
         goAlert.alertInfo("Insert User", data.message);
         isInsertUserClose = true;
 
         function fnInsertStepUserPreClose() {
             rpt_user_housekeeping_fn_insert_process_step(
                 "InsertUserPreClose",
                 "Y",
                 rpt_user_housekeeping_fn_get_date_time_now().toString(),
                 "button load insert user pre close and user resign",
                 "3",
                 "2",
                 "N/A",
                 () => {
                     setTimeout(function () {
                         statusInsertStepUserClose = true;
                     }, 500);
                     console.log("insert step insert user pre-close complete");
                 }
             );
         }
 
         function fnInsertUser() {
             RptBIHkpFnInsertProcessStat(
                 "InsertUserPreClose",
                 "1",
                 rpt_user_housekeeping_fn_get_date_time_now().toString(),
                 "Insert process insert user pre-close complete.",
                 "0",
                 "2",
                 data.insertCount.toString(),
                 function () {
                     setTimeout(function () {
                         statusInsertProcessUserClose = true;
                     }, 500);
                     console.log("Insert process insert user pre close complete");
                 }
             );
         }
 
         function fnGetStatus() {
             return statusInsertStepUserClose && statusInsertProcessUserClose;
         }
         fnCallNextProcessUserHousekeeping(
             fnInsertStepUserPreClose,
             () => isInsertUserClose,
             500,
             10,
             "Insert step user pre-close"
         );
         fnCallNextProcessUserHousekeeping(
             fnInsertUser,
             () => statusInsertStepUserClose,
             500,
             10,
             "Insert process user pre-close"
         );
         fnCallNextProcessUserHousekeeping(
             fnGetProcessStepUserHousekeeping,
             fnGetStatus,
         ...
         goAlert.alertError("Error Insert User", data.message);
     }
 } */
 
 //function RptBIHkpFnGetOldUserPreClose() {
 //  let reportDate = $("#rpt_bi_hkp_bi_deletion_date_in").val();
 //  CallAPI.Go(
 //    url_get_user_bi_close_user_housekeeping,
 //    {
 //      reportDate: reportDate,
 //    },
 //    RptBIHkpFnGetOldUserPreCloseCallback,
 //    "processing"
 //  );
 //}
 
 //let oldUserPreClose = "";
 //function RptBIHkpFnGetOldUserPreCloseCallback(data) {
 //  if (data.status === "1") {
 //    goAlert.alertInfo("Get old user pre-close", data.message);
 //    rpt_user_housekeeping_apply_data_to_table_bi_user_close(
 //      data.data.listStaff
 //    );
 //    oldUserPreClose = data;
 ...
 //function RptBIHkpFnGetProcessStatus(date) {
 //  CallAPI.Go(
 //    vBIHkpGetProcessStatus,
 //    {
 //      reportDate: date,
 //    },
 //    RptBIHkpFnGetProcessStatusCloseCallback,
 //    "processing"
 //  );
 //}
 
 //function RptBIHkpFnGetProcessStatusCloseCallback(data) {
 //  if (data.status === "1") {
 //  } else {
 //    goAlert.alertError("Error Process", data.message);
 //  }
 ...
 //async function RptBIHkpFnGetUserCloseCallback(data) {
 //  if (data.status == "1") {
 //    dataUserClose = data;
 //    isLoadDataFromExcel = true;
 //    fn_tab_change_user_housekeeping("pre_close_user_tab");
 //    rpt_user_housekeeping_apply_data_to_table_bi_user_close(
 //      data.data.listStaff
 //    );
 //    // create json to store in db
 //    const jsonUserClose = JSON.stringify(data.data.listStaff);
 //    // insert step load user from excel file
 //    rpt_user_housekeeping_fn_insert_process_step(
 //      "7",
 //      "Y",
 //      rpt_user_housekeeping_fn_get_date_time_now().toString(),
 //      "get user from excel success",
 //      jsonUserClose.toString(),
 //      function () {
 //        setTimeout(function () {
 //          staInsertStepLoadUserFromExcel = true;
 //        }, 500);
 //        console.log("Insert step load user complete");
 //      }
 //    );
 //    // insert process user from excel
 //    await fnCallNextProcessUserHousekeeping(
 //      FnInsertProcessGetUserFromExcel,
 //      () => staInsertStepLoadUserFromExcel,
 //      500,
 //      10,
 //      "Insert user from excel."
 //    );
 //    // insert process user bi close
 //    await fnCallNextProcessUserHousekeeping(
 //      FnInsertUserPreClose,
 //      () => staInsertProLoadUserFromExcel,
 //      500,
 //      10,
 //      "Insert user BI close"
 //    );
 ...
 //        staInsertStepLoadUserFromExcel &&
 //        staInsertProLoadUserFromExcel &&
 //        staInsertUserCloseFromExcel
 //      );
 //    }
 //    // Retry call for the next process
 //    await fnCallNextProcessUserHousekeeping(
 //      fnGetProcessStepUserHousekeeping,
 //      fnRefreshStatus,
 //      500,
 ...
 //    );
 //  } else {
 //    goAlert.alertError("Process Failed", data.message);
 //  }
 //}
 
 //=======================DB House keeping===========================
 function RptDBHkpFnRefreshListing() {
   let dateRage = $("#rang_date_id_user_housekeeping").val();
   let fromDate = dateRage.split(" - ")[0];
   let toDate = dateRage.split(" - ")[1];
 }
 ...
 function fnOpenModalOperationBIUserHousekeeping() {
   modals.Open("modal_operation_bi_user_housekeeping");
   //listingBIUserHousekeeping
   fn_apply_data_to_table_bi_user_inactive_user_housekeeping(
     listingBIUserHousekeeping.data,
